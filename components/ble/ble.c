...
int BLE_NOTIFY_ENABLED = 0;
esp_gatt_if_t GATT_IF = 0;

void example_write_event_env(esp_gatt_if_t gatts_if, prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param);
void example_exec_write_event_env(prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param);

static void ble_tx_send(char *payload, uint16_t length)
{
    ...
    int len = length < 20 ? length : 20;
    uint8_t data[len];
    memcpy(data, payload, len);
    esp_ble_gatts_send_indicate(GATT_IF,
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
        ...
        if (param->update_conn_params.status == 0x10 && gl_profile_tab[PROFILE_A_APP_ID].updt < 3)
        {
            gl_profile_tab[PROFILE_A_APP_ID].updt++;
            gl_profile_tab[PROFILE_A_APP_ID].conn_params.min_int += 0x10;
            gl_profile_tab[PROFILE_A_APP_ID].conn_params.max_int += 0x10;
            ESP_LOGI(GATTS_TAG, "updating gap parameters");
            esp_ble_gap_update_conn_params(&gl_profile_tab[PROFILE_A_APP_ID].conn_params);
        }
        break;
    default:
        break;
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_MTU_EVT:
        ...
        if (param->mtu.mtu != 500)
        {
            esp_err_t err = esp_ble_gatt_set_local_mtu(param->mtu.mtu);
            if (err)
            {
                ESP_LOGE(GATTS_TAG, "set local  MTU failed, error code = %x", err);
            }
        }
        break;
    ...
    case ESP_GATTS_CONNECT_EVT:
    {
        ...
        gl_profile_tab[PROFILE_A_APP_ID].conn_params = conn_params;
        gl_profile_tab[PROFILE_A_APP_ID].updt = 0;
        //start sent the update connection parameters to the peer device.
        esp_ble_gap_update_conn_params(&conn_params);
        robot_connected((tx_func)ble_tx_send);
        break;
    }
